    ...
    with codecs.open(eval_file_path, encoding='utf-8') as flist:
        ...
        for line in lines:
            ...
            if str(result) == parts[1]:
                right_count += 1
        period = time.time() - t1
        print("Total evaluation count: {}".format(total_count))
        print("Correct evaluation count: {}".format(right_count))
        # print("Cost time: {} sec".format("%.2f" % period))
        print("Predict accuracy: {:.2f}%".format(right_count / total_count * 100))
